...
player = Actor("d:\수업\오픈소스\OSS\pacman2\images\pacman_o.png") # Load in the player Actor image
player.score = 0            
player.lives = 3
level = 0
SPEED = 3
...
def getPlayerImage():
    ...
    if tc > 2.5 and (player.movex != 0 or player.movey !=0):
        if a != 180:
            player.image = "d:\수업\오픈소스\OSS\pacman2\images\pacman_c"
        else:
            player.image = "d:\수업\오픈소스\OSS\pacman2\images\pacman_cr"
    else:
        if a != 180:
            player.image = "d:\수업\오픈소스\OSS\pacman2\images\pacman_o"
        else:
            player.image = "d:\수업\오픈소스\OSS\pacman2\images\pacman_or"
    player.angle = a
...
def runAway(g, dirs): #도망(전체해당)
    ...
    if player.y < ghosts[g].y and dirs[3] == 1 and not aboveCentre(ghosts[g]): ghosts[g].dir = 1
